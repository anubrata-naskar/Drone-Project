...
def ant_colony_optimization(num_trucks, capacity, x_coords, y_coords, demands, num_ants=20, iterations=100, alpha=1, beta=2, evaporation_rate=0.5, Q=100):
    """
    Improved Ant Colony Optimization for Vehicle Routing Problem
    
    Ensures each node is visited exactly once by a single truck
    """
    ...
    # Best routes for each truck
    best_truck_routes = []
    
    # Track visited nodes across all trucks
    global_unvisited = set(range(1, num_nodes))
    
    # Construct routes for each truck
    for truck in range(num_trucks):
        truck_pheromones = pheromones.copy()
        truck_best_routes = []
        truck_best_distances = []
        
        # Local unvisited nodes for this truck (subset of global unvisited)
        local_unvisited = global_unvisited.copy()
        
        for _ in range(iterations):
            # Store ant routes for this truck in this iteration
            ant_routes = []
            ant_route_distances = []
            
            for _ in range(num_ants):
                # Generate a route for an ant (truck)
                route = [0]  # Start at depot
                current_node = 0
                current_load = 0
                
                # Copy local unvisited for this ant's route
                route_unvisited = local_unvisited.copy()
                ...
                while route_unvisited:
                    ...
                    for node in route_unvisited:
                        # Check capacity constraint
                        if current_load + demands[node] <= capacity:
                            # Calculate probability based on pheromone and distance
                            pheromone = truck_pheromones[current_node][node] ** alpha
                            distance_factor = (1 / (distances[current_node][node] + 1e-10)) ** beta
                            probabilities.append(pheromone * distance_factor)
                            valid_nodes.append(node)
                    
                    # If no valid nodes, break or return to depot
                    if not valid_nodes:
                ...
                ant_route_distances.append(route_distance)
            
            # Update pheromones
            # Evaporation
            truck_pheromones *= (1 - evaporation_rate)
            
            # Deposit pheromones
            for route, distance in zip(ant_routes, ant_route_distances):
                for i in range(len(route) - 1):
                    truck_pheromones[route[i]][route[i+1]] += Q / distance
                    truck_pheromones[route[i+1]][route[i]] += Q / distance
            
            # Track best solution for this truck
            min_route_distance = min(ant_route_distances)
            best_route_index = ant_route_distances.index(min_route_distance)
            truck_best_routes.append(ant_routes[best_route_index])
            truck_best_distances.append(min_route_distance)
        
        # Select the best route for this truck
        best_truck_route_index = truck_best_distances.index(min(truck_best_distances))
        best_route = truck_best_routes[best_truck_route_index]
        
        # Remove visited nodes from global unvisited set
        route_nodes = set(best_route) - {0}  # Exclude depot
        global_unvisited -= route_nodes
        
        best_truck_routes.append(best_route)
    
    return best_truck_routes
...
truck_routes = ant_colony_optimization(num_trucks, capacity, x_coords, y_coords, demands)
