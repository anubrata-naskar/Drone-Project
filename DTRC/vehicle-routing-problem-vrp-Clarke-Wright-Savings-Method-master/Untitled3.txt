        ...
        while remain_nodes:
            if available_drones:
                ...
                for node in remain_nodes:
                    if (self.nodes[node-1].demand <= self.drone_capacity and 
                        self.distances[drone_positions[drone]-1][node-1] <= drone_batteries[drone]):
                        cost = self.distances[drone_positions[drone]-1][node-1]
                        if cost < min_cost:
                            min_cost = cost
                            best_node = node
