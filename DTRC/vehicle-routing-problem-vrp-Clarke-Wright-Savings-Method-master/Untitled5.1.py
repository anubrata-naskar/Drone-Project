...
def i_k_means(num_trucks, capacity, x_coords, y_coords, demands, num_iterations=100):
    ...
    def construct_cluster_route(cluster_indices):
        ...
        while unvisited:
            ...
            # If no valid nodes, return to depot
            if not valid_nodes:
                route.append(0)
                break
...
def calculate_total_cost(truck_routes, all_drone_routes, distances_df, demands):
    ...
    truck_cost = total_cost
    print("Only truck cost - ", truck_cost)    
    ...
    return total_cost, delivery_cost, truck_cost, drone_cost, waiting_cost

def convert_to_int(obj):
    """Recursively convert numpy integers to Python integers"""
...
def format_routes_for_csv(routes):
    """Format routes nicely for CSV display"""
    # Convert all NumPy integers to regular integers recursively
    clean_routes = convert_to_int(routes)
    # Convert to a nice string format without extra spaces
    return str(clean_routes).replace(" ", "")

def process_multiple_files(file_paths, output_csv_path, drone_nodes_dict=None):
    """
    Process multiple CVRP files and save results to a CSV file
    
    Parameters:
    file_paths (list): List of file paths to process
    output_csv_path (str): Path to save the CSV output
    drone_nodes_dict (dict): Dictionary mapping file names to drone node configurations
    """
    # Default drone nodes configuration if not provided
    if drone_nodes_dict is None:
        drone_nodes_dict = {}
        # Default configuration for each file
        for file_path in file_paths:
            filename = os.path.basename(file_path)
            drone_nodes_dict[filename] = {1: [12, 15], 4: [7, 11]}
    ...
    for file_path in file_paths:
        ...
        # Get drone nodes configuration for this file
        drone_nodes = drone_nodes_dict.get(filename, {1: [12, 15], 4: [7, 11]})
        ...
        try:
            ...
            drone_routes, updated_truck_routes = apply_dtrc(truck_routes, drone_nodes, distances_df, demands)
            
            print("-----Updated Truck routes-----")
            print(updated_truck_routes)
            print("-----Drone routes-----")
            print(drone_routes)
            
            # Calculate costs
            total_cost, delivery_cost, truck_cost, drone_cost, waiting_cost = calculate_total_cost(
                updated_truck_routes, drone_routes, distances_df, demands
            )
            
            # Store results with cleaned route formatting
            results.append({
                'filename': filename,
                'truck_routes': format_routes_for_csv(updated_truck_routes),
                'drone_routes': format_routes_for_csv(drone_routes),
            ...
            results.append({
                'filename': filename,
                'truck_routes': f"ERROR: {str(e)}",
                'drone_routes': f"ERROR: {str(e)}",
                'truck_cost': 0,
                'drone_cost': 0,
                'waiting_cost': 0
            })
    ...
    with open(output_csv_path, 'w', newline='') as csvfile:
        fieldnames = ['filename', 'truck_routes', 'drone_routes', 'truck_cost', 'drone_cost', 'waiting_cost']
...
if __name__ == "__main__":
    ...
    # Custom drone nodes configuration for each file
    drone_nodes_dict = {
        "A-n32-k5.vrp": {1: [12, 15], 4: [7, 11]},
        "A-n33-k5.vrp": {1: [12, 15], 3: [7, 11]},
        "A-n34-k5.vrp": {2: [13, 16], 4: [8, 12]},
        # Add more configurations as needed
    }
    
    # Output CSV file path
    output_csv = "cvrp_results.csv"
    
    # Process all files
    process_multiple_files(input_files, output_csv, drone_nodes_dict)