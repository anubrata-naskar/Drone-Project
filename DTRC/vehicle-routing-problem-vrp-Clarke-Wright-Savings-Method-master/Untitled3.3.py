...
def apply_dtrc(truck_routes, drone_nodes, distances_df, demands):
    ...
    for truck_route in truck_routes:
        ...
        for takeoff_node in possible_takeoff_nodes:
            ...
            for candidate in drone_nodes[takeoff_node]:
            ...
            if delivery_node:
                # Find a valid landing node (must be in the same truck route)
                landing_node = None
                for truck_node in truck_route[1:-1]:  # Exclude depot
                    if truck_node != takeoff_node and truck_node not in used_nodes:
                        landing_node = truck_node
                        break

                if landing_node:
                    drone_routes[drone_idx].append([takeoff_node, delivery_node, landing_node])
                    used_nodes.update([takeoff_node, delivery_node])

                    # Ensure the truck visits takeoff first, then lands after drone delivery
                    if takeoff_node in truck_route and landing_node in truck_route:
                        takeoff_index = truck_route.index(takeoff_node)
                        landing_index = truck_route.index(landing_node)
                        if takeoff_index > landing_index:  
                            truck_route.remove(takeoff_node)
                            truck_route.insert(landing_index, takeoff_node)  

                    # Ensure the truck skips the drone-delivered node
                    if delivery_node in truck_route:
                        truck_route.remove(delivery_node)
