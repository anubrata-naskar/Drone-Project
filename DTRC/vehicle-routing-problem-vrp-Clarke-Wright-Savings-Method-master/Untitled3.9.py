...
class VRPSolver:
    def __init__(self, file_path):
        """
        Initialize the VRP Solver with a VRP file path.
        
        Args:
            file_path (str): Path to the VRP file.
        """
        self.file_path = file_path
        self.num_trucks, self.capacity, self.x_coords, self.y_coords, self.demands = self.read_cvrp_file()
        self.distances_df = self.distance_matrix_from_xy()
        self.truck_routes = None
        self.drone_routes = None
        
    def read_cvrp_file(self):
    ...
    def distance_matrix_from_xy(self):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                dist = math.sqrt((self.x_coords[i] - self.x_coords[j])**2 + (self.y_coords[i] - self.y_coords[j])**2)
                dist_matrix[i][j] = dist
                dist_matrix[j][i] = dist

        return pd.DataFrame(dist_matrix)
    
    def battery_model(self, total_payload):
        """Battery model for drone range calculation."""
        return (155885 / ((200 + total_payload) ** 1.5))
    
    def calculate_route_cost(self, route, dist_matrix):
        ...
        while improved:
            ...
            for i in range(1, len(route) - 2):
                for j in range(i + 1, len(route)):
                    ...
                    if self.calculate_route_cost(new_route, dist_matrix) < self.calculate_route_cost(best, dist_matrix):
                        best = new_route
                        improved = True
            route = best
        return best

    def simple_relocate(self, route, dist_matrix):
        """Simple relocate optimization for route improvement."""
        best = route
    ...
    def swap_move(self, route, dist_matrix):
        """Swap move optimization for route improvement."""
        best = route
        for i in range(1, len(route) - 1):
            for j in range(i + 1, len(route)):
    ...
    def clarke_wright_savings(self):
        ...
        nodes = pd.DataFrame({'d0': distances_df.iloc[:, 0], 'demand': [self.demands[i + 1] for i in range(len(self.demands))]})
        ...
        for link in sv.index:
            ...
            if route_indices[0] == route_indices[1] == -1:
                if sum_cap([n1, n2]) <= self.capacity:
                    routes.append([n1, n2])
                    node_list.remove(n1)
                    node_list.remove(n2)
                ...
                if sum_cap(routes[route_idx] + [n2]) <= self.capacity:
                    routes[route_idx].append(n2)
                    node_list.remove(n2)

            elif route_indices[0] == -1 and route_indices[1] != -1:
                route_idx = route_indices[1]
                if sum_cap(routes[route_idx] + [n1]) <= self.capacity:
                    routes[route_idx].append(n1)
                    node_list.remove(n1)

            elif route_indices[0] != -1 and route_indices[1] != -1 and route_indices[0] != route_indices[1]:
                if sum_cap(routes[route_indices[0]] + routes[route_indices[1]]) <= self.capacity:
        ...
        for route in routes:
            ...
            # Apply optimizations
            optimized_candidate_routes = [
                self.two_opt(route, distances_df),
                self.simple_relocate(route, distances_df),
                self.swap_move(route, distances_df)
            ]
        
            # Select the route with the minimum cost
            best_route = min(optimized_candidate_routes, key=lambda r: self.calculate_route_cost(r, distances_df))

            optimized_routes.append(best_route)

        return optimized_routes
    
    def apply_dtrc(self, truck_routes, drone_nodes):
        """Apply Drone-Truck Routing Coordination algorithm."""
            ...
            while i < len(truck_route) - 1:  # Until the last node before returning to depot
                ...
                for drone_idx in range(2):
                    ...
                    for j in range(i + 1, len(truck_route) - 1):
                        ...
                        weight = self.demands.get(candidate_node+1, self.demands.get(candidate_node, 0))
                        
                        # Only include if within drone weight capacity (35 units)
                        if weight <= 35:
                            D = self.distances_df.iloc[current_node, candidate_node]
                    ...
                    if potential_deliveries:
                        ...
                        for num_nodes in range(1, max_nodes + 1):
                            for combo in [potential_deliveries[:num_nodes]]:  # Just take the first num_nodes
                                ...
                                max_distance = self.battery_model(total_payload)
                                ...
                                for landing_idx in range(max(indices) + 1, len(truck_route)):
                                    ...
                                    for k in range(len(complete_route) - 1):
                                        total_distance += self.distances_df.iloc[complete_route[k], complete_route[k+1]]
    ...
    def plot_truck_routes(self, truck_routes):
        ...
        for idx, route in enumerate(truck_routes):
            for i in range(len(route) - 1):
                plt.arrow(self.x_coords[route[i]], self.y_coords[route[i]], 
                          self.x_coords[route[i+1]] - self.x_coords[route[i]], 
                          self.y_coords[route[i+1]] - self.y_coords[route[i]], 
                          head_width=0.5, length_includes_head=True, color='blue', alpha=0.8)
            
            # Plot truck stops
            plt.scatter([self.x_coords[node] for node in route], 
                        [self.y_coords[node] for node in route], 
                        color='blue', marker='o', alpha=0.7, label="Truck Route" if idx == 0 else None)

        # Plot depot
        plt.scatter(self.x_coords[0], self.y_coords[0], color='black', marker='s', s=100, label="Depot")
    ...
    def plot_combined_routes(self, truck_routes, all_drone_routes, drone_nodes):
        ...
        for idx, route in enumerate(truck_routes):
            ...
            # Plot truck stops
            plt.scatter([self.x_coords[node] for node in route_nodes], 
                        [self.y_coords[node] for node in route_nodes], 
                        color=truck_color, marker='o', alpha=0.7, 
                        label=f"Truck {idx+1} Route" if idx == 0 else f"Truck {idx+1}")
            
            # Label nodes
            for node in route_nodes:
                plt.text(self.x_coords[node], self.y_coords[node], str(node), fontsize=8, 
            ...
            for drone_idx, drone_route_list in enumerate(truck_drone_routes):
                for route in drone_route_list:
                    ...
                    for next_node in route[1:]:  # Start from first delivery node
                        plt.arrow(self.x_coords[prev_node], self.y_coords[prev_node], 
                                 self.x_coords[next_node] - self.x_coords[prev_node], 
                                 self.y_coords[next_node] - self.y_coords[prev_node], 
                                 head_width=0.5, length_includes_head=True, color=drone_color, 
                                 linestyle='dashed', alpha=0.7)
                        prev_node = next_node
                    
                    # Add a small annotation to show which drone it is
                    mid_x = (self.x_coords[takeoff] + self.x_coords[landing]) / 2
                    mid_y = (self.y_coords[takeoff] + self.y_coords[landing]) / 2
        ...
        # Takeoff nodes
        if takeoff_nodes:
            plt.scatter([self.x_coords[node] for node in takeoff_nodes], 
                        [self.y_coords[node] for node in takeoff_nodes], 
                        color='green', marker='^', s=100, label="Takeoff Points")
        
        # Landing nodes
        if landing_nodes:
            plt.scatter([self.x_coords[node] for node in landing_nodes], 
                        [self.y_coords[node] for node in landing_nodes], 
                        color='purple', marker='v', s=100, label="Landing Points")
        
        # Delivery-only nodes (nodes only visited by drones)
        if delivery_only_nodes:
            plt.scatter([self.x_coords[node] for node in delivery_only_nodes], 
                        [self.y_coords[node] for node in delivery_only_nodes], 
        ...
        # Plot depot
        plt.scatter(self.x_coords[0], self.y_coords[0], color='black', marker='s', s=100, label="Depot")
        plt.text(self.x_coords[0], self.y_coords[0], "0", fontsize=10, color='white',
    ...
    def calculate_total_cost(self, truck_routes, all_drone_routes):
        ...
        for route_idx, route in enumerate(truck_routes):
            route_cost = self.calculate_route_cost(route, self.distances_df)
            total_cost += route_cost
            
            # Calculate arrival times for each node in the truck route
            current_time = 0
            for i in range(len(route) - 1):
                current_time += self.distances_df.iloc[route[i], route[i+1]]
        ...
        for truck_idx, truck_drone_routes in enumerate(all_drone_routes):
            ...
            for drone_idx, drone_route_list in enumerate(truck_drone_routes):
                for route in drone_route_list:
                    ...
                    payload = 0
                    for node in delivery_nodes:
                        weight = self.demands.get(node+1, self.demands.get(node, 0))
                        payload += weight
                    
                    # Calculate max distance based on payload
                    max_distance = self.battery_model(payload)
                    ...
                    for i in range(len(route) - 1):
                        segment_distance = self.distances_df.iloc[route[i], route[i+1]]
        ...
        for route in truck_routes:
            if len(route) > 2:  # Only if route has at least one delivery point
                ...
                for i in range(1, len(route) - 2):  # Start at first delivery, end before last delivery
                    delivery_cost += self.distances_df.iloc[route[i], route[i+1]]
            ...
            for drone_route_list in truck_drone_routes:
                for route in drone_route_list:
                    ...
                    for i in range(len(route) - 1):
                        ...
                        delivery_cost += self.distances_df.iloc[route[i], route[i+1]] / 1.5
        
        return total_cost, delivery_cost
    
    def solve(self, drone_nodes=None):

        # If no drone nodes provided, use a default
        if drone_nodes is None:
            drone_nodes = {1: [12, 15], 4: [7, 11]}
        
        # First solve the VRP using Clarke-Wright Savings algorithm
        self.truck_routes = self.clarke_wright_savings()
        
        print("-----Truck routes after Clarke-Wright Savings-----")
        for i, route in enumerate(self.truck_routes):
            print(f"Truck {i+1}: {route}")
        
        # Plot truck routes
        self.plot_truck_routes(self.truck_routes)
        
        # Apply DTRC algorithm
        self.drone_routes, self.truck_routes = self.apply_dtrc(self.truck_routes, drone_nodes)
        ...
        for i, truck_drones in enumerate(self.drone_routes):
            ...
            for j, drone_routes in enumerate(truck_drones):
                print(f"  Drone {j+1}: {drone_routes}")
        
        # Plot combined routes
        self.plot_combined_routes(self.truck_routes, self.drone_routes, drone_nodes)
        
        # Calculate costs
        total_cost, delivery_cost = self.calculate_total_cost(self.truck_routes, self.drone_routes)
        print(f"\nTotal Cost: {total_cost:.2f}")
        print(f"Delivery Cost: {delivery_cost:.2f}")
        
        return total_cost, delivery_cost, self.truck_routes, self.drone_routes

# Example usage
if __name__ == "__main__":
    # Create an instance of the solver
    solver = VRPSolver("A-n32-k5.vrp")
    
    # Solve the problem
    total_cost, delivery_cost, truck_routes, drone_routes = solver.solve()