...
def tabu_search_vrp(num_trucks, capacity, x_coords, y_coords, demands, max_iterations=100, tabu_tenure=5):
    ...
    # Initial solution generation using a greedy approach
    def generate_initial_solution():
        # Prepare list of nodes (excluding depot)
        ...
        # Distribute nodes to trucks
        current_truck = 0
        current_load = 0
        
        for node in all_nodes:
            # If adding node exceeds capacity, move to next truck
            if current_load + demands[node] > capacity:
                current_truck += 1
                current_load = 0
                
                # Stop if we've used all trucks
                if current_truck >= num_trucks:
                    break
            
            # Add node to current truck's route
            routes[current_truck].append(node)
    ...
    # Swap two nodes between different routes
    def inter_route_swap(routes, i, j, x, y):
        new_routes = [route.copy() for route in routes]
        
        # Swap nodes
        new_routes[i][x], new_routes[j][y] = new_routes[j][y], new_routes[i][x]
        ...
        while improved:
            ...
            for i in range(1, len(route) - 2):
                for j in range(i + 1, len(route) - 1):
                    ...
                    old_dist = sum(distances[route[k]][route[k+1]] for k in range(len(route)-1))
                    new_dist = sum(distances[new_route[k]][new_route[k+1]] for k in range(len(new_route)-1))
    ...
    def tabu_search():
        ...
        # Tabu list to prevent recent moves
        tabu_list = []
        
        # Track best solution distance
        best_distance = calculate_total_distance(current_solution)
        ...
        for iteration in range(max_iterations):
            ...
            for i in range(num_trucks):
                for j in range(i+1, num_trucks):
                    ...
                    for x in range(1, len(current_solution[i]) - 1):
                        for y in range(1, len(current_solution[j]) - 1):
                            # Create candidate solution
                            candidate = inter_route_swap(current_solution, i, j, x, y)
                            
                            # Check route validity
                            if (is_route_valid(candidate[i]) and 
                                is_route_valid(candidate[j])):
            ...
            for solution in neighborhood:
                ...
                for i in range(num_trucks):
                    optimized_solution[i] = two_opt(solution[i])
                local_search_solutions.append(optimized_solution)
            
            # Select best non-tabu move
            best_candidate = None
            best_candidate_distance = float('inf')
            
            for candidate in local_search_solutions:
                candidate_distance = calculate_total_distance(candidate)
                
                # Check if move is tabu
                is_tabu = False
                for tabu_move in tabu_list:
                    if candidate == tabu_move:
                        is_tabu = True
                        break
                
                # Aspiration criteria: accept if better than global best
                if (not is_tabu or candidate_distance < best_distance) and candidate_distance < best_candidate_distance:
...
truck_routes = tabu_search_vrp(num_trucks, capacity, x_coords, y_coords, demands)
