...
def k_means_routing(num_trucks, capacity, x_coords, y_coords, demands):
    # Get distance matrix
    distances_df = distance_matrix_from_xy(x_coords, y_coords)
    
    # Extract coordinates for all nodes except depot (node 0)
    ...
    cluster_nodes = [[] for _ in range(num_trucks)]
    for i, cluster_id in enumerate(clusters):
        # Add 1 because we excluded depot (node 0) from clustering
        node_id = i + 1
        cluster_nodes[cluster_id].append(node_id)
    
    # Build optimized routes for each cluster
    optimized_routes = []
    
    for cluster_id, nodes in enumerate(cluster_nodes):
        # Check capacity constraint
        total_demand = sum(demands[node] for node in nodes)
        
        # If cluster exceeds capacity, need to split (simplified approach)
        if total_demand > capacity:
            # Sort nodes by distance from depot
            depot_distances = [(node, distances_df.iloc[0, node]) for node in nodes]
            depot_distances.sort(key=lambda x: x[1])
            
            # Create multiple routes by greedy bin packing
            current_route = []
            current_demand = 0
            
            for node, _ in depot_distances:
                node_demand = demands[node]
                
                if current_demand + node_demand <= capacity:
                    current_route.append(node)
                    current_demand += node_demand
                else:
                    # Complete current route
                    route = [0] + current_route + [0]
            ...
            # Add remaining nodes as the last route
            if current_route:
                route = [0] + current_route + [0]
            ...
            # Simple case: cluster fits in one truck
            route = [0] + nodes + [0]  # Start and end at depot
    ...
    # If we have fewer routes than trucks, create additional routes
    while len(optimized_routes) < num_trucks:
        # Find the route with the most nodes
        route_idx = max(range(len(optimized_routes)), 
                         key=lambda i: len(optimized_routes[i]))
        
        # Get the route and remove depot visits
        route = optimized_routes[route_idx][1:-1]
        
        if len(route) <= 2:
            # Not enough nodes to split
            break
            
        # Split the route roughly in half
        mid = len(route) // 2
        route1 = route[:mid]
        route2 = route[mid:]
        
        # Create two new routes with depot
        new_route1 = [0] + route1 + [0]
        new_route2 = [0] + route2 + [0]
        
        # Apply optimizations
        route1_options = [
            two_opt(new_route1, distances_df),
            simple_relocate(new_route1, distances_df),
            swap_move(new_route1, distances_df)
        ]
        
        route2_options = [
            two_opt(new_route2, distances_df),
            simple_relocate(new_route2, distances_df),
            swap_move(new_route2, distances_df)
        ]
        
        # Select best routes
        best_route1 = min(route1_options, key=lambda r: calculate_route_cost(r, distances_df))
        best_route2 = min(route2_options, key=lambda r: calculate_route_cost(r, distances_df))
        
        # Replace the original route and add the new one
        optimized_routes[route_idx] = best_route1
        optimized_routes.append(best_route2)
    
    # If we have more routes than trucks, merge routes
    while len(optimized_routes) > num_trucks:
        ...
        for i in range(len(optimized_routes)):
            for j in range(i + 1, len(optimized_routes)):
                # Calculate "distance" between routes as minimum distance between any nodes
                min_route_distance = float('inf')
                
                for node_i in optimized_routes[i][1:-1]:  # Skip depot
                    for node_j in optimized_routes[j][1:-1]:  # Skip depot
                        if distances_df.iloc[node_i, node_j] < min_route_distance:
                            min_route_distance = distances_df.iloc[node_i, node_j]
                
                if min_route_distance < min_distance:
                    min_distance = min_route_distance
                    merge_indices = (i, j)
        
        # Get the routes to merge
        route1 = optimized_routes[merge_indices[0]][1:-1]  # Remove depot
        route2 = optimized_routes[merge_indices[1]][1:-1]  # Remove depot
        
        # Check capacity constraint
        total_demand = sum(demands[node] for node in route1 + route2)
        
        if total_demand <= capacity:
            # Merge routes if capacity allows
            merged_route = [0] + route1 + route2 + [0]
            
            # Apply optimizations
            route_options = [
                two_opt(merged_route, distances_df),
                simple_relocate(merged_route, distances_df),
                swap_move(merged_route, distances_df)
            ]
            ...
            optimized_routes = [optimized_routes[i] for i in range(len(optimized_routes)) 
                                if i != merge_indices[0] and i != merge_indices[1]]
            optimized_routes.append(best_route)
        else:
            # If we can't merge further due to capacity constraints, break the smallest route
            # and distribute its nodes to other routes
            routes_by_size = sorted(range(len(optimized_routes)), 
                                   key=lambda i: len(optimized_routes[i]))
            smallest_route_idx = routes_by_size[0]
            
            # Get nodes from the smallest route (excluding depot)
            nodes_to_distribute = optimized_routes[smallest_route_idx][1:-1]
            
            # Remove the smallest route
            optimized_routes.pop(smallest_route_idx)
            
            # Try to add each node to the best possible route
            for node in nodes_to_distribute:
                node_demand = demands[node]
                best_insertion = (float('inf'), None, None)  # (cost_increase, route_idx, position)
                
                for i, route in enumerate(optimized_routes):
                    # Check capacity constraint
                    route_demand = sum(demands[n] for n in route[1:-1])
                    
                    if route_demand + node_demand <= capacity:
                        # Try inserting at each position
                        for pos in range(1, len(route)):
                            new_route = route[:pos] + [node] + route[pos:]
                            cost_increase = calculate_route_cost(new_route, distances_df) - calculate_route_cost(route, distances_df)
                            
                            if cost_increase < best_insertion[0]:
                                best_insertion = (cost_increase, i, pos)
                
                # Insert the node at the best position
                if best_insertion[1] is not None:
                    route_idx, pos = best_insertion[1], best_insertion[2]
                    optimized_routes[route_idx] = optimized_routes[route_idx][:pos] + [node] + optimized_routes[route_idx][pos:]
                else:
                    # If we can't add the node to any route, create a new one
                    optimized_routes.append([0, node, 0])
            
            # If we've distributed all nodes but still have too many routes, just stop
            if len(optimized_routes) <= num_trucks:
                break
...
truck_routes = k_means_routing(num_trucks, capacity, x_coords, y_coords, demands)
