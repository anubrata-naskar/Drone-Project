...
class TruckDroneRouting:
    def __init__(self, truck_routes, drone_routes, demands,
                 ...
                 time_weight=0.5, force_use_drones=True, num_drones=5):
        """
    ...
    def destroy_solution(self, truck_routes, drone_routes):
        ...
        # Filter out moves that create intersections between truck routes
        removed_nodes = self.filter_conflicting_moves(truck_routes, removed_nodes)
        ...
        for item in removed_nodes:
            ...
            for op, _ in operations:
                ...
                if solution and obj_value < best_objective:
                    # Check if the new route creates an intersection with existing truck routes
                    if not self.check_intersection(current_truck_routes, solution[0]):
    ...
    def filter_conflicting_moves(self, truck_routes, removed_nodes):
      """
      Filter out moves that create intersections between truck routes
      """
      filtered_nodes = []
      for node in removed_nodes:
          if isinstance(node, tuple) and len(node) >= 2:
              node_type, node_id = node[0], node[1]
          else:
              continue  # Skip if unknown format

          if node_type == 'truck':
              # Find the route that contains the node
              for route in truck_routes:
                  if node_id in route:
                      # Check if the removed node creates an intersection with existing truck routes
                      if not self.check_intersection(truck_routes, node_id, route):
                          filtered_nodes.append(node)
                      break

      return filtered_nodes

    def check_intersection(self, truck_routes, node, route):
      """
      Check if a new node creates an intersection with existing truck routes
      """
      node_index = route.index(node)
      if node_index == 0 or node_index == len(route) - 1:
          return False

      prev_node = route[node_index - 1]
      next_node = route[node_index + 1]

      # Create LineString objects for the line segment
      line_segment_ls = LineString([self.node_locations[prev_node], self.node_locations[next_node]])

      for r in truck_routes:
          if len(r) < 2:
              continue

          # Create LineString objects for the routes
          route_ls = LineString([self.node_locations[n] for n in r])

          # Check if the line segment intersects with the route
          if line_segment_ls.intersects(route_ls):
              return True
