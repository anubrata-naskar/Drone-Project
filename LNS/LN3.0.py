...
class TruckDroneRouting:
    def __init__(self, truck_routes, drone_routes, demands,
                 road_distance_matrix, euclidean_distance_matrix,
                 max_iterations=1000, p1=0.3, p2=0.3, p3=0.3,
                 drone_capacity=100, drone_cost_factor=0.8,
                 truck_speed=12, drone_speed=80,
                 time_weight=0.5, force_use_drones=True):
        """
        ...
        Args:
            ...
            road_distance_matrix: Matrix of road distances between nodes for trucks
        ...
        self.road_distance_matrix = road_distance_matrix
        ...
        self.truck_speed = truck_speed  # km/h
        self.drone_speed = drone_speed  # km/h
    ...
    def _validate_input(self):
        ...
        for i, truck_drones in enumerate(self.drone_routes):
            if len(truck_drones) > 2:
                raise ValueError(f"Truck {i} has more than 2 drones assigned")

        # Validate distance matrices
        expected_size = self.num_nodes
        if self.road_distance_matrix.shape != (expected_size, expected_size):
    ...
    def truck_distance(self, node1, node2):
        """Calculate road distance between two nodes for truck"""
        return self.road_distance_matrix[node1, node2]
    ...
    def calculate_route_cost(self, route, is_drone=False):
        ...
        # Apply drone cost factor if it's a drone route
        if is_drone:
            cost = distance * self.drone_cost_factor
        else:
            cost = distance
        ...
        for i, route in enumerate(truck_routes_copy):
            ...
            for j in range(len(route) - 1):
                ...
                if self.calculate_drone_load(new_drone_route) <= self.drone_capacity:
                    ...
                    for d in range(len(drone_routes_copy[i])):
    ...
    def repair_solution(self, truck_routes, drone_routes, removed_nodes):
        ...
        and selecting the best insertion for each node
        """
        ...
        for truck_idx, _, node in eligible_nodes:
            ...
            if node in route:
                ...
                if pos > 0 and pos < len(route) - 1:
                    ...
                    if self.calculate_drone_load(new_drone_route) <= self.drone_capacity:
                        # Find an available drone
                        for j in range(len(drone_routes_copy[truck_idx])):
                            # Find an empty slot
...
if __name__ == "__main__":
    # Create sample data
    num_nodes = 29  # Including depot 0
    ...
    road_distance_matrix = pd.read_csv('road_distances.csv', header=0, index_col=0).values
    ...
    for i in range(num_nodes):
        for j in range(num_nodes):
            ...
            road_distance_matrix[i, j] = dist * detour_factor

    # Parse input
    truck_routes_input = "[[0, 23, 26, 15, 0], [0, 21, 12, 0], [0, 25, 19, 0], [0, 8, 16, 0]]"
    drone_routes_input = "[[[[23, 10, 27, 3, 22]], [[26, 20, 2, 22, 26]]], [[[21, 11, 13, 18, 21]], [[12, 5, 24, 12]]], [[[25, 9, 1, 7]], [[19, 7, 19]]], [[[8, 4, 14, 6, 8]], [[16, 17, 16]]]]"
    truck_speed=12
